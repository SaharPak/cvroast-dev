...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (env.RATE_LIMIT) {
      const used = await env.RATE_LIMIT.get(rateLimitKey);
      if (used) {
        return new Response(
          JSON.stringify({
            error: 'rate_limited',
            message: "You've used your free roast for today. Try again in 24 hours, or fork the open-source skill on GitHub for unlimited use: https://github.com/SaharPak/ai-cv-roast",
          }),
    ...
    // Mark this IP as having used today's free roast
    if (env.RATE_LIMIT) {
    ...
    return new Response(
      JSON.stringify({ markdown, used_today: true }),
